...
public class HandleExceptions {
	...
	public ResponseEntity<String> resourcesNotFoundExceptionException(ResourcesNotFoundException ex) {
		return ResponseEntity.status(HttpStatus.NOT_FOUND) // 413
				.body(ex.getMessage());
	}
	...
	public ResponseEntity<?> handleUserNtfound(UserNotFoundException ex) {
	...
	public ResponseEntity<String> handleAllExceptions(Exception ex) {
		...
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
				.body("ஒரு பிழை ஏற்பட்டுள்ளது. பின்னர் மீண்டும் முயற்சிக்கவும்.");
	}
